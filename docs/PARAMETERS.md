...
| `LYRICS_WHISPER_API_URL`                    | Fork-only. When set, ASR transcription is sent to this external OpenAI-compatible `/v1/audio/transcriptions` endpoint (e.g. a self-hosted faster-whisper-server/speaches instance) instead of running the bundled local ONNX Whisper model. Leave empty (default) to keep the local pipeline. See `lyrics/whisper_onnx.py`'s module docstring for what the remote path does and doesn't preserve. | `""` |
| `LYRICS_WHISPER_API_MODEL`                  | Fork-only. Model name sent to the remote transcription endpoint.                          | `Systran/faster-whisper-small`         |
| `LYRICS_WHISPER_API_KEY`                    | Fork-only. Bearer token for the remote transcription endpoint, if it requires one.        | `no-key-needed`                        |
